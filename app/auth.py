...
def create_magic_link(session: Session, email: str) -> str:
    token = secrets.token_urlsafe(32)
    link = MagicLink(email=email, token=token, expires_at=datetime.utcnow() + MAGIC_LINK_TTL)
    session.add(link)
    session.commit()
    return token
...
def verify_token(session: Session, token: str) -> Employee:
